...
"$ROOT/scripts/build-sample-app.sh" "$PLATFORM"

if [[ "$PLATFORM" == "android" ]]; then
  export ANDROID_HOME="${ANDROID_HOME:-$HOME/Library/Android/sdk}"
  export PATH="$ANDROID_HOME/platform-tools:$ANDROID_HOME/emulator:$PATH"
  adb devices | grep -q emulator || (echo "Start an Android emulator first." && exit 1)
  SUITE="testng-android.xml"
  MVN_ARGS=(-Dplatform=android -DsuiteXmlFile="$SUITE")
else
  DEVICE="${IOS_DEVICE:-iPhone 17}"
  xcrun simctl boot "$DEVICE" 2>/dev/null || true
  open -a Simulator 2>/dev/null || true
  SUITE="testng-ios.xml"
